function [W, funcVal] = f_GMTM_APG(X, Y,opts,C)
%% 
...
%% 10-15-2012, add sparse group lasso.
if isfield(opts, 'rho_L4')
    rho_L4 = opts.rho_L4;    
else
    rho_L4 = 0;
end
...
while iter < opts.maxIter
    ...
    while true
        ...
        % 10-15-2012 add lasso item.
        if rho_L4==0
            Wzp = FGLasso_projection(Ws - gWs/gamma, rho1 / gamma);
        else
            Us= l1_projection(Ws - gWs/gamma, 2 *  rho_L4 / gamma);  
        ...
        Fzp_gamma = Fs + trace(delta_Wzp' * gWs)...
            + gamma/2 * norm(delta_Wzp, 'fro')^2;  % the smooth function value of Q(w,wt) after iteration
...
%10-15-2012 add the sparse group lasso.
...
%         grad_W = grad_W+ rho_L2 * 2 * W;
        % modify:
        for i = 1:task_num
            grad_W = cat(2, grad_W, X{i}*(X{i}' * W(:,i)-Y{i})+rho_L3*C{i}*W(:,i) );
        end
        grad_W = grad_W+ rho_L2 * 2 * W;
...
% smooth part function value.
    function [funcVal] = funVal_eval (W)
        ...
        % modify manifold value
        for i=1: task_num
            funcVal = funcVal + 0.5*rho_L3 * W(:,i)'*C{i}*W(:,i);
        end
    ...
    function [non_smooth_value] = nonsmooth_eval(W, rho_1)
       ...
       % 10-15-2012, add lasso item.
        non_smooth_value=non_smooth_value+rho_L4*sum(sum(abs(W)));
